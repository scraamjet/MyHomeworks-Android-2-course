...
import com.example.homework.domain.helpers.LayoutHelper
...
class WeatherInfoActivity : AppCompatActivity(),CoroutineScope {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        launch {
            val weatherResponse =
                ApiFactory.getWeatherById(intent.getIntExtra("id", 0))
            ...
            this@WeatherInfoActivity?.runOnUiThread {
                ...
                constraintLayoutWeatherInfoActivity.setBackgroundResource(
                    layoutHelper.getLayoutBackgroundDrawableId(backgroundDrawableName, R.drawable::class.java))
                Glide.with(applicationContext).load(iconUri).into(iv_weather_icon)

                tv_city_temp.text = "${temperatureConverter.convert((weatherResponse.main.temp).toInt())} "
                ...
                tv_feels_like.text = "Feels like: ${temperatureConverter.convert(
